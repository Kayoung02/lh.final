...
from common.public_supply import (
    ...
    summarize_by_agency,
    summarize_by_developer_type,
    summarize_by_district,
)


def _filter_public_supply(public_supply: pd.DataFrame) -> pd.DataFrame:
    st.sidebar.header("1. 공공 시행 공급지도 필터")
    districts = sorted(public_supply.loc[public_supply["시군구"].ne("미상"), "시군구"].unique())
    agencies = sorted(public_supply["시행사_표시"].dropna().unique())
    years = sorted(public_supply["사용승인연도"].dropna().astype(int).unique())
    ...
    selected_types = st.sidebar.multiselect(
        "시행주체 구분",
        sorted(public_supply["시행주체 구분"].dropna().unique()),
        default=sorted(public_supply["시행주체 구분"].dropna().unique()),
    )
    selected_years = st.sidebar.slider(
        "사용승인 연도", min_value=min(years), max_value=max(years), value=(min(years), max(years))
    )

    filtered = public_supply.copy()
...
def render_public_supply_map() -> None:
    st.subheader("공공 시행 공급지도")
    st.caption("점은 단지이며, 확대하면 단지별 시행사·세대수·사용승인 정보를 확인할 수 있습니다.")
    ...
    public_supply = get_public_supply_data(apartment)
    filtered = _filter_public_supply(public_supply)

    total_households = int(filtered["세대수"].sum())
    total_buildings = int(filtered["동수"].sum())
    valid_points = int(filtered["좌표유효"].sum())
    kpi1, kpi2, kpi3, kpi4 = st.columns(4)
    kpi1.metric("공공 관련 단지", f"{len(filtered):,}개")
    kpi2.metric("총 세대수", f"{total_households:,}세대")
    kpi3.metric("총 동수", f"{total_buildings:,}개동")
    kpi4.metric("지도 표시 가능", f"{valid_points:,}개")

    st.markdown("#### 시행주체 유형별 공급 비율")
    developer_type_summary = summarize_by_developer_type(filtered)
    ratio_base = st.radio("비율 기준", ["세대수", "단지수"], horizontal=True, key="public_ratio_base")
    ...
    with st.expander("지도 해석 및 다음 단계"):
        st.markdown(
            "- 기본 집계에는 `공공`, `기타공공`, `공공·조합 공동`, `공공·민간 공동`이 포함됩니다.\n"
            "- `세대수`와 `단지수` 기준의 비율을 바꿔 보면 대규모 단지 중심 공급과 단지 수 중심 공급의 차이를 확인할 수 있습니다.\n"
